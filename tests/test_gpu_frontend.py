...
def test_the_ir_carries_the_kinds_and_the_source_backends_write_them(tmp_path: Path):
    ...
    assert (
        "__global__ void ppy_gpu_prog_saxpy(int64_t a0, double a1, const double *a2, double *a3)"
        in source
    )
    assert "static __device__ double ppy_gpu_prog_fma(double a0, double a1, double a2)" in source
    ...
    assert (
        ...
        and "cudaDeviceSynchronize() == cudaSuccess" in source
    )
    assert "goto fallback;" not in source.split("ppy_gpu_prog_saxpy(")[1].split("__global__")[0]
    ...
    assert (
        ...
        and "hipDeviceSynchronize() == hipSuccess" in hip_text.stdout
    )

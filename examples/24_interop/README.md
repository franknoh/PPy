...
- If `geometry.py` and `geometry.ppy` both exist, the `.ppy` wins and a
  `PPyAmbiguousModuleWarning` says so.
- `ppy check` rejects the ambiguity outright (`E1003`), which is why
  `ppy convert --in-place` removes the `.py` it replaces.
- `ppy convert` inserts `import ppy` ahead of first-party imports, so a
  converted entry point can import its siblings.
...
- `PPY_IMPORT=python` turns it off for a process.

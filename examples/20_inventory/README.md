...
parameter and return typed, `Sequence` where the body only reads, and
`@ppy.pure` where the checker proved it. `examples/verify_conversions.py`
...
No tool makes that decision. It is a design change, not a mechanical one,
and the converter renames nothing and splits nothing.

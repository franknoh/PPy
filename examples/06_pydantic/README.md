...
Pydantic models are typed by the plugin and still validated by pydantic at
run time.
...
because pydantic coerces the string. `user.id + 1` is `int` arithmetic,
because the checker types the field from the model, not from the argument.
Validation still runs as pydantic wrote it.
...
the same refinement `ppy.Range(0, 255)` gives. Inside `luminance` the three
fields are known to fit a byte, and the arithmetic needs no overflow guard.

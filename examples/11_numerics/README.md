...
PPy gives the same answers as CPython for overflow, floor division, and the
sign of the remainder, cases where several other native compilers differ.
...
- Rust in debug is the only other one that notices the overflow, and its
  answer is to stop.

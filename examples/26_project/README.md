...
- `app.ppy` calls `ppy.install()` before importing its sibling, so the same
  file runs under plain `python` too.
- `ppy convert src/` and `ppy check src/` take the directory and work across
  it.

...
| `05_numpy` | elementwise NumPy fused into one kernel; against NumPy, numexpr, Numba, JAX |
...
| `09_torch` | a function of tensor ops as one C++ ATen region; against PyTorch eager and `torch.compile` |
...
| `13_value_classes` | an all-scalar dataclass with no boxed form; against Numba's `@jitclass` |
| `14_tuples` | fixed tuples as scalar ABI atoms; against Numba |
...
| `37_aio` | an echo server and its client as `ppy.aio` coroutines, native runtime or asyncio; against asyncio and uvloop |
...
| `40_generics` | type parameters, bounds, and Protocols, monomorphized in native code; against Numba |
| `41_columnar` | pandas Series expressions fused as one loop over the columnar dialect; against pandas and polars |

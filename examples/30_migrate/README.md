...
A small legacy telemetry script, run through `ppy migrate` instead of
`ppy convert`. The script is dynamic in the usual harmless ways:
`importlib.import_module`, a `globals()` write, and `setattr`/`getattr` on
constant names.
...
to the static form it always meant, and proves the rewrite equivalent before
making it. Once nothing uses it, it removes the `import importlib` that fed
the first line.

Then the ordinary conversion runs over the rewritten code. It adds:

- fourteen annotations
- `Final` on the constant
- `Sequence` where a parameter is only read
- `@ppy.pure` on `spread`

The result passes `ppy check` under `strict = true` with nothing left over.
This migration ends where `ppy convert` starts.
...
- what was `AUTOFIXED`
- what `REQUIRES_REWRITE`
- what needs a `DYNAMIC_BOUNDARY`
- whether the output is `strict_ready`

`--diff` shows the rewrite before it happens.

## On a real codebase

On a real codebase the unit of migration is a kernel file, not the whole

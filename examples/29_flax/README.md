# A Flax training loop, strict-checked as written

`train.py` trains an MLP regression with Flax (linen) and optax: a
`nn.Module` subclass, `@nn.compact`, `optax.adam`, a `@partial(jax.jit)`
training step. `train.ppy` is what `ppy convert` wrote from it, and it
checks under `strict = true` with nothing extra installed or configured.
The jax plugin models the Flax and optax surface, so strict mode has a
...
receives a `jax.Array` because that is what flows into it — and
`@ppy.pure` where the checker proved it. `train_step`, decorated with
...
type-check. Parameter pytrees are an explicit `Any` boundary, declared as
such in the source, not an inferred one. All three paths train to the same
loss on the same device, matching the original `.py`.
...
- [Training with JAX](../22_training_jax/README.md) — the same shape without Flax.
- [Plugins: JAX](../../docs/internals/plugins.md) — the Flax and optax surface.

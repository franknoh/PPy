...
row-major order; everything else is arithmetic on the values, and says
nothing about memory. `lower-tensor` decides that.
"""
...
def _verify_elementwise(op: Operation, checker: Checker) -> None:
    ...
    if op.local_name == "div" and not isinstance(a.dtype, FloatType):
        checker.error(op, "tensor.div takes floating-point tensors")
    try:

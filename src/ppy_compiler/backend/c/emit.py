...
def emit_module(
    ...
    *,
    ...
    fails, so the text is a whole program.
    """
    return _ModuleEmitter(module, language, header_only, entry).run()


class _ModuleEmitter:
    def __init__(
        self, module: IRModule, language: Language, header_only: bool, entry: str | None
    ) -> None:
        self.module = module
        self.language = language
        self.cpp = language is Language.CPP
    ...
    def assemble(self) -> str:
        cpp = self.cpp
        lines = [f"/* {self.module.name}: generated by ppy, {'C++17' if cpp else 'C11'} */"]
        libraries = self.module.attributes.get("ppy.libraries", ())
        if libraries:
            flags = " ".join(f"-l{name}" for name in libraries)  # type: ignore[union-attr]
        ...
        for header in sorted(headers):
            if cpp and header.endswith(".h") and header != "inttypes.h":
...
class _FunctionEmitter:
    ...
    def op(self, op: Operation) -> None:
        ...
        if op.dialect == "math":
            ...
            return
        if op.dialect != "core":
            raise EmitError(f"{op.name}: the C backend lowers the core and math dialects")
        match name:
            case "const":

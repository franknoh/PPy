...
| `llvm.sanitize` | `[]` | sanitizers to instrument every build with: `bounds`, `overflow`, `pointer`, `alignment`; `--sanitize` overrides for one run. Selects the IR road. |
| `llvm.pgo` | unset | a `.ppyprof` from `ppy run --profile`, relative to the project root, that guides every build; `--pgo FILE` overrides for one run. Selects the IR road. |

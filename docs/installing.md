...
This page covers installing the `ppy-lang` package, its optional extras,
the native toolchain it uses, and platform support.
...
The distribution is `ppy-lang`. It installs three packages:

- `ppy`: the runtime and the directives
- `ppy_compiler`
- `ppy_runtime`
...
The base package contains the compiler and the runtime. Extras enable the
rest, so you install only what you use.
...
compiler on `PATH`. That includes the small C scanner that reads input, the
Python-ABI wrappers, the native objects, and standalone executables.

- **C compiler** (`cc` or `gcc`): needed for the complete native path.
  Without one, the boundary falls back to the slower ctypes trampoline and
  `W2004` says so once.
- **CPython headers** (`python3-dev`): the fastest `METH_FASTCALL` boundary
  is built against them. An interpreter installed by `uv python install
  3.13` brings its headers with it.
- **GPU**: running a `ppy.cuda` kernel on a device needs the CUDA driver, a
  device, and libdevice (the CUDA toolkit, `CUDA_HOME` or `PPY_LIBDEVICE`).
...
- **XLA**: the PJRT bridge that runs an `@xla.jit` function on a device
  ...
  StableHLO does not.
- **Coroutines**: the native runtime behind `ppy.aio` is built once into
  the cache on Linux (epoll) with a C compiler. On other platforms asyncio
...
Build where you run. Don't copy a built artifact to an older machine: a
library linked against glibc 2.35 does not load on glibc 2.27. `ppy doctor`
prints the libc it found.
...
earlier. Python 3.14 needs `libcst` 1.8 and runs on machines new enough
for it.
...
- a `uv`-managed interpreter brings its headers, so the fast boundary is
  available without a system `python3-dev`
...
Plugin runtimes are separate dependency groups, so you install only what
you intend to test: `uv sync --group torch`, `--group jax`, `--group
uvicorn`, `--group scipy`, `--group pandas`, `--group pyarrow`, `--group
all`. [Contributing](contributing.md) has the rest.
